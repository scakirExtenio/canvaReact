...
const { CANVA_FRONTEND_PORT, CANVA_BACKEND_PORT, CANVA_BACKEND_HOST, CANVA_APP_ID, CANVA_HMR_ENABLED, CANVA_ORGINAL_S3BUCKET_URL, CANVA_THUMBNAIL_S3BUCKET_URL, CANVA_DEFAULT_VIDEO_THUMBNAIL } =
	process.env;
...
const BACKEND_URL = `http${SHOULD_ENABLE_HTTPS ? "s" : ""}://localhost:${CANVA_BACKEND_PORT}`;

if (HMR_ENABLED && APP_ID.length === 0) {
	throw new Error("CANVA_HMR_ENABLED environment variable is TRUE, but CANVA_APP_ID is not set. Refer to the instructions in the README.md on configuring HMR.");
}
...
async function start() {
	...
	console.log(`Starting development server for`, chalk.greenBright.bold(ENTRY_DIR));
	console.log("");

	if (!HMR_ENABLED) {
		console.log("HMR not enabled. To enable it, please refer to the instructions in the", chalk.greenBright.bold("README.md"));
		console.log("");
	}
	...
	if (!fs.existsSync(frontendEntry)) {
		throw new Error(`Entry point for frontend does not exist: ${frontendEntry}`);
	}
	...
	if (fs.existsSync(developerBackendEntry)) {
		if (!CANVA_APP_ID) {
			throw new Error(`'CANVA_APP_ID' environment variable is undefined. Refer to the instructions in the README.md on starting a backend example.`);
		}
		...
		let url;
		try {
			url = SHOULD_RUN_NGROK ? await ngrok.connect(CANVA_BACKEND_PORT) : BACKEND_URL;
	...
	table.push(["Development URL (Frontend)", chalk.cyan(getFrontendUrl(runtimeWebpackConfig.devServer.server.type))]);

	console.log(table.toString());
	console.log("");

	if (SHOULD_RUN_NGROK) {
		console.log(chalk.bold.bgYellow("Warning:"), `ngrok exposes a local port via a public URL. Be mindful of what's exposed and shut down the server when it's not in use.`);
		console.log("");
	}

	console.log(`${chalk.blue.bold("Note:")} For instructions on how to set up the app via the Developer Portal, see ${chalk.bold(path.join(ROOT_DIR, "README.md"))}.`);
